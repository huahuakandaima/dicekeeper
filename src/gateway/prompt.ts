...
- 输出格式要求 JSON 外壳，但 narrative 字段内部必须是纯自然语言叙事：禁止在叙事文本中出现任何 JSON 结构、工具调用结果原文（如 {"dice":...}、{"ok":true...}）、代码块或字段名（narrative/dice_results/prompt_player 等），不得把工具返回的 JSON 原样复述给玩家。
